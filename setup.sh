...
if ! [ -d "$BACKUP_DIR" ] || ! [ -d "$BACKUP_DIR/config" ];
then
...
then
  ...
  backed=0
  linked=0
  for node in "$CONFIG_DIR"/* "$CONFIG_DIR"/.*; \
  do
    ...
    elif [[ "$package" == "$(basename "$IGNORE_FILE")" ]];
    then
      continue
      
    # skip if the package is tracked by git (not a new package)
    elif git ls-files --error-unmatch "$target" &> /dev/null;
    then
      continue
    fi
    ...
    then
      ...
      if ! grep -qF "$package" "$IGNORE_FILE";
      then
  ...
  if [ -z "$linked_state" ] || [ -z "$backed_state" ];
  then
    echo "$linked" >> "$setup_cache"
    echo "$backed" >> "$setup_cache"

  else
    if [ "$linked" -gt "$linked_state" ];
    then
      sed -i '' "2s/.*/$linked/" "$setup_cache"
    fi
    if [ "$backed" -gt "$backed_state" ];
    then
