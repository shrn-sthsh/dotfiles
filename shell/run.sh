...
# Bash completion
if [[ $OSTYPE == "darwin"* ]] && [[ -r "$(brew --prefix)/etc/profile.d/bash_completion.sh" ]];
then
...
if [ -d "$dotfiles/shell/util/" ];
then
  for script in "$dotfiles/shell/util/"*.sh;
  do
    if [ -f "$script" ];
    then
      source "$script"
...
# Set environment
if [ -f "$dotfiles/shell/env.sh" ];
then
  source "$dotfiles/shell/env.sh"
fi

# Load modules
if [ -d "$dotfiles/shell/mod/" ];
then
  for script in "$dotfiles/shell/mod/"*.sh;
  do
    if [ -f "$script" ];
    then
      source "$script"
...
then
  ...
  linked="$(sed -n '2p' "$state" | tr -d '[:space:]')"
  backed="$(sed -n '3p' "$state" | tr -d '[:space:]')"
